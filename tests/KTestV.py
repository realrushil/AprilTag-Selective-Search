...
Tag = TrackTag()
for q in range(5, 6):
    k = round(0.0416667*q*q*q*q - 0.583333*q*q*q + 2.95833*q*q - 2.41667*q+1)
    wname = str(k) + '_K-V-long.txt'
    file = open(wname, 'w')
    ...
    for i in range(1, 5):
        ...
        for j in range(1, 4):
            ...
            avgFPS = frame_count / (endTot - startTot)
            file.write(str(avgFPS) + '\n')
            cap.release()
            cv2.destroyAllWindows()
            print(fname + ' done!' + '(' + str(k) + ')')
